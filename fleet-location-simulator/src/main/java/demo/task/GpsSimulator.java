...
public class GpsSimulator implements Runnable{
    ...
    private boolean exportPositionsToMessaging = false;

    private Integer reportInterval = 500; //milli seconds at which to send position reports
            ...
            while (!Thread.interrupted()) {
                ...
                if (positionInfo != null) {
                    ...
                    final CurrentPosition currentPosition = new CurrentPosition(
                            positionInfo.getVin(),
                            new Point(positionInfo.getPosition().getLatitude(), positionInfo.getPosition().getLongitutde()),
    ...
    /**
     * On thread interrupt. Send null position to all consumers to indicate that sim has closed.
     */
    void destroy() {
        positionInfo = null;
    }
    ...
    void moveVehicle() {
        ...
        Double excess = 0.0; //amount by which next position will exceed end point of present leg
